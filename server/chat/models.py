...
class Message (models.Model):
    ...
    date = models.DateTimeField (default = timezone.now())      # get the current time the message was sent
    user = models.CharField (max_length = 100)                  # name of the user who sent the message
    room = models.ForeignKey (Room, on_delete = models.CASCADE) # room is a foreign key here and if the room is deleted any messages in that room will simultaneously be deleted
